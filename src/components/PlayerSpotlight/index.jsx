      ...
      try {
        const response = await axios.get(
          "https://corsmirror.onrender.com/v1/cors?url=" +
            encodeURIComponent(`https://api-web.nhle.com/v1/player-spotlight`)
        );
          ...
          .map((player) => (
            <Link to={`/player/${player.playerId}`} key={player.playerId}>
              <div className="m-4 bg-gray-700 hover:bg-gray-600 cursor-pointer text-white shadow-xl rounded-lg overflow-hidden">
                <img
                  className="w-full h-64 object-cover shadow-xl rounded-full"
                  src={player.headshot}
                ...
                <div className="p-4">
                  ...
                  <span className="flex flex-row justify-center">
                    <img
                      className="w-12 h-12 border-r-2 border-gray-500"
                      src={player.teamLogo}
